...
Source of intent: [session prompt 2](prompt%20log.md#prompt-2). This run's milestone
is initialization, design, a minimal executable scaffold, dependency configuration,
and initial hardware-free tests. It is not physical commissioning or a release.
...
| REQ-007 | Serial transactions are serialized. Communication failures produce typed errors and explicit unavailable/stale state; no write retry or reconnect silently reapplies a setpoint. Explicit disconnect is idempotent. | TEST-002, TEST-005, TEST-006 fault/lifecycle tests |
| REQ-008 | A background monitoring service samples without a browser or Dash callback. **Derived:** default 1 s interval, configurable positive finite interval; a 0.02 s test interval yields at least 3 samples within 2 s; stop joins cleanly. | TEST-006 monitor tests |
...
- No unattended write retry or automatic reconnect in the initial scaffold.
  Operator/application reconnect is explicit; reads may continue on a later poll.
- The milestone may finish in SOFTWARE_DEVELOPMENT while further hardware-free
  robustness and integration work remains; it must not be labeled hardware-ready.
